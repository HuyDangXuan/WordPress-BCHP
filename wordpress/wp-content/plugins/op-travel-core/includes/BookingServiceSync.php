...
final class BookingServiceSync
{
    ...
    public static function sync_order($order_id, $posted_data, $order)
    {
        ...
        if ($endpoint === '') {
            $order->add_order_note(__('BOOKING_SERVICE_ENDPOINT is missing. Booking sync skipped.', 'op-travel-core'));
            return;
        }
        ...
        if (is_wp_error($response)) {
            $order->add_order_note(sprintf(
                __('Booking sync failed: %s', 'op-travel-core'),
        ...
        if ($status_code < 200 || $status_code >= 300 || ! is_array($body)) {
            $order->add_order_note(__('Booking sync returned an invalid response.', 'op-travel-core'));
            return;
        }
